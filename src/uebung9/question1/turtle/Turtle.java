...
public class Turtle {
	static int[][][] picture = new int[300][500][3];
	static {
		erase();
	}
	...
	private static void putPixel(int x, int y) {
		...
		if (yMax - y - 1 < 0 || yMax - y - 1 >= picture.length || x < 0
				|| x >= picture[0].length || !isDrawing) {
			return;
		}
	...
	public static void forward(double n) {
		...
		drawLine((int) Math.round(x), (int) Math.round(y),
				(int) Math.round(xn), (int) Math.round(yn));
		x = xn;
		y = yn;
