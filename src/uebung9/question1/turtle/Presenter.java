...
public class Presenter {
	...
	// */
	// public static void viewImage(String title, int[][] grayScaleImage, int i)
	// {
	// int[][][] rgbImage = new
	// int[grayScaleImage.length][grayScaleImage[0].length][3];
	//
			...
			if (grayScaleImage != null) {
				for (int y = 0; y < grayScaleImage.length; y++) {
					for (int x = 0; x < grayScaleImage[0].length; x++) {
						rgbImage[y][x] = ImageGrabber.ImageGrabberUtilities
								.convertYCbCrToRGB(
										(double) grayScaleImage[y][x] / 256.0,
										0.5, 0.5);
					}
				}
			}
